...
def remove_outliers(df, column_name):
    ...
    Author: https://www.geeksforgeeks.org/detect-and-remove-the-outliers-using-python/
    """
...
def plot_feature_distrib(df):
    """Plots the distribution of features for treatment and control groups.

    Args:
        df (pd.DataFrame): Movie dataframe with identified treatment and control groups.
    """
    # Mapping the treatment column from 0,1 to Treatment, Control
    df['Group'] = df['treatment'].map({0: 'Treatment', 1: 'Control'})
    
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5), sharex=True)
    
    # Plot for movie release year distribution
    sns.violinplot(data=df, x='Group', y='movie_release_year', ax=ax1)
    ax1.set_title("Movies' Release Year Distributions")
    ax1.set_xlabel('Group')
    ax1.set_ylabel('Release Year')

    # Plot for movie language count distribution
    sns.violinplot(data=df, x='Group', y='movie_languages_count', ax=ax2)
    ax2.set_title("Movies' Language Count Distributions")
    ax2.set_xlabel('Group')
    ax2.set_ylabel('Number of Languages')
...
def create_naive_analysis_table():
    """
    Create and style a Pandas DataFrame table representing an analysis of cast diversity impact on movie performance metrics.

    Returns:
        styled_table (pandas.io.formats.style.Styler): A styled Pandas DataFrame table with conditional formatting.
    ...
    index = [
        'Ethnic diversity of the cast and the movie\'s box office revenue',
        'Ethnic diversity of the cast and the movie\'s rating',
        'Gender diversity of the cast and the movie\'s box office revenue',
        'Gender diversity of the cast and the movie\'s rating'
    ]

    table = pd.DataFrame(data, index=index)

    table.iloc[0] = ['Positive and significant'] * 2
    table.iloc[1] = ['Negligibly negative and significant'] * 2
    table.iloc[2:] = ['Null or non significant']

    def apply_color(val):
        if val == 'Positive and significant':
            return 'color: green; background-color: rgba(144, 238, 144, 0.5)'
        if val == 'Negligibly negative and significant':
            return 'color: red; background-color: rgba(255, 200, 200, 0.5)'
        else:
            return ''

    styled_table = table.style.applymap(apply_color)
    styled_table.set_caption("Naive analysis of the Cast Diversity Impact on the Movie Performance Metrics")

    return styled_table

def create_global_causal_analysis_table():
    """
    Create and style a Pandas DataFrame table representing an analysis of cast diversity impact on movie performance metrics.
    ...
    table.iloc[0] = ['Positive and significant']
    table.iloc[1] = ['Null or non significant']
    table.iloc[2] = ['Negligibly negative and significant']
    table.iloc[3] = ['Negligibly negative and significant']
    ...
    styled_table = table.style.applymap(apply_color)
    styled_table.set_caption("Global causal analysis of the Cast Diversity Impact on the Movie Performance Metrics")

    return styled_table


def create_years_causal_analysis_table():
    """
    Create and style a Pandas DataFrame table representing an analysis of cast diversity impact on movie performance metrics.
    ...
    table.iloc[0] = ['Positive and significant only after 1970']
    table.iloc[1] = ['Negative and significant between 2005 and 2008']
    table.iloc[2:] = ['Null or non significant']

    def apply_color(val):
        if val == 'Positive and significant only after 1970':
            return 'color: green; background-color: rgba(144, 238, 144, 0.5)'
        if val == 'Negative and significant between 2005 and 2008':
            return 'color: red; background-color: rgba(255, 200, 200, 0.5)'
        else:
            return ''
